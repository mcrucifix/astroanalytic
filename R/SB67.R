#' importFrom stats ts
#' importFrom gtseries cis
#' importFrom graphics par lines grid
#' importFrom gtseries as.data.frame plot
...
#' Internal function for Sharaf-Budnikova precession model
#'
#' This function implements the core trigonometric expansions for obliquity, longitude of the
#' vernal equinox, and climatic precession, used internally by \code{\link{SB67}}.
#' Users normally do not call this function directly.
#'
#' @param PlanetarySolution list containing the planetary solution:
...
#' @param P0 numeric, partial derivative of precession with respect to eccentricity, in radians
#' @param ell numeric, "newcomb" constant, in radians
#' @param epsilonbar numeric, mean obliquity at reference year, in radians
#' @param zeta numeric, precession phase at reference year, in radians
#' @param aggregating logical, aggregate terms with identical frequencies? Default `TRUE`.
#' @param ber78_strict logical, enforce Berger (1973) original implementation. Default `FALSE`.
#' @param ber90_reproduce logical, hack to exactly reproduce Berger & Loutre (1991). Default `FALSE`.
#'
#' @return A named list of three \code{discreteSpectrum} objects:
#' \describe{
#'   \item{OBLIQUITY}{Amplitudes, frequencies, and phases for obliquity.}
#'   \item{PSI}{Amplitudes, frequencies, and phases for longitude of vernal equinox.}
#'   \item{CLIMPRECESS}{Amplitudes, frequencies, and phases for climatic precession.}
#' }
#' Each element includes optional attributes:
#' \describe{
#'   \item{shift}{Phase reference relative to zero.}
#'   \item{trend}{For PSI, the mean linear trend.}
#' }
#'
#' @details
#' This routine is a lower-level implementation of the Sharaf-Budnikova precession model.
#' It is primarily used by \code{SB67} and should not be called directly by end-users.
#' Aggregation is recommended for series evaluation to reduce redundancy.
#'
#' @seealso \code{\link{SB67}}
#'
#' @keywords internal
SB67_Internal <- function(ell, P0, epsilonbar, zeta = 1.600753 * pi / 180,
  ...
  # DISABLED : WAS USED FOR TESTING
  # COULD GO BACK AS AN EXAMPLE
  ## require(palinsol)
  ## data(BER90)
...
#' @param ell numeric, "newcomb" constant, in radians
#' @param epsilonbar numeric, mean obliquity at reference year, in radians
#' @param zeta numeric, precession phase at reference year, in radians. Warning: phase reference differs between Laskar (2000) and Bretagnon (1950)
#' @param aggregating logical, should terms with the same frequency be aggregated? Default is `TRUE`.
#' @param ber78_strict logical, enforce original implementation of Berger (1973) thesis. Default `FALSE`.
#' @param ber90_reproduce logical, hack to reproduce Berger and Loutre (1991) results exactly. Default `FALSE`.
#' @return A list with three `discreteSpectrum` objects:
